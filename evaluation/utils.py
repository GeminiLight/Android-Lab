...
def clone_avd(src_avd_name, tar_avd_name, android_avd_home):
    ...
    if not os.path.exists(tar_avd_dir):
        # Exclude live instance lock files: cloning a *running* AVD would
        # otherwise carry multiinstance.lock over and make the cloned
        # emulator abort with "Running multiple emulators with the same AVD".
        shutil.copytree(
            src_avd_dir,
            tar_avd_dir,
